...
public class SparkIcebergController {
  ...
  ResponseEntity<String> atlasTest() throws AtlasServiceException {
    ...
    //    AtlasEntity.AtlasEntityWithExtInfo ksootHiveDb =
    //        this.atlasClient.getEntityByAttribute("hive_db", Map.of("qualifiedName",
    // "ksoot@primary"));
    //    AtlasEntity.AtlasEntityWithExtInfo driverHourlyStatsTable =
    //        this.atlasClient.getEntityByAttribute(
    //            "hive_table", Map.of("qualifiedName", "ksoot.driver_hourly_stats@primary"));
  ...
  @ApiResponses(
      value = {
        @ApiResponse(responseCode = "200", description = "Iceberg tables deleted successfully"),
  ...
  ResponseEntity<String> createOrUpdateTables() {
    ...
    Table driverStatsTable =
        icebergCatalogClient.createOrUpdateTable(
            ...
            Set.of(
                ...
                IcebergField.optional(
                    "avg_daily_trips", Types.IntegerType.get(), "Average Daily Trips")));
    ...
    //            "created",
    //            Set.of(
    //                IcebergField.optional("current_balance", Types.FloatType.get(), "Current
    // Balance"),
    //                IcebergField.optional("avg_passenger_count", Types.FloatType.get(), "Average
    // Passenger Count"),
  ...
  ResponseEntity<List<String>> printSchema() {
    ...
    //    String schemaRepresentationCustomerProfilesTable =
    // customerProfilesTable.schema().toString();
    //    System.out.println(
    //        "Schema Representation of Table "
    //            + TABLE_NAME_CUSTOMER_DAILY_PROFILE
    //            + ": "
    ...
    // schemaRepresentationCustomerProfilesTable));
    return ResponseEntity.ok(List.of(schemaRepresentationDriverStatsTable));
  }

  @Operation(operationId = "dump-data", summary = "Dump Data into CSV files")
  @ApiResponses(
      value = {
        @ApiResponse(responseCode = "200", description = "Data written to CSV files successfully"),
        @ApiResponse(responseCode = "400", description = "Bad request"),
        @ApiResponse(responseCode = "500", description = "Internal Server Error")
      })
  ...
  ResponseEntity<String> dumpData() {
    ...
    return ResponseEntity.ok("Data written to CSV files successfully");
  }

  @Operation(
      operationId = "spark-pipeline",
      summary = "Execute Spark pipeline to read data from CSV and write into Iceberg Tables")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  //    System.out.println("----------------------------------------------");
  //    try {
  //      // Replace with the full class name
  //      Class<?> clazz =
  // Class.forName("org.apache.spark.sql.execution.datasources.v2.AppendDataExec");
  //
  //      // Get all declared methods (including private, protected)
  //      Method[] declaredMethods = clazz.getDeclaredMethods();
  //
  //      System.out.println("Declared methods in class " + clazz.getName() + ":");
  //      for (Method method : declaredMethods) {
  //        System.out.println(method.toString());
  //      }
  //
  //      // Optional: to get only public methods (including inherited)
